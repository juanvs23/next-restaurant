...
export default function Modal() {
  const { openModal, modal } = useAppSelector(
    (state: RestoState) => state.modal,
  );
      ...
      <div className="modal-overlay">
        <div className="modal">
          <div className="modal-header">
            ...
            <button className="closer" onClick={() => resetModal()}>
              <LiaTimesSolid />
            </button>
          </div>
          <div className="modal-body">{modal.modalContent}</div>
          <div className="modal-footer">{modal.ModalFooter}</div>
        </div>
      </div>
    </motion.div>
  );
